...
async def connect_dashboard(
    ...
    # ── Extract from Tableau (multi-strategy: CSV → REST → Image) ────────
    df       = pd.DataFrame()
    snap     = None
    if view_id and check_tableau():
        try:
            ...
            # Use robust multi-strategy extraction — handles ANY visual type
            snap = extractor.extract_with_fallback(
                target      = target,
                source_name = source_name,
                filters     = filters or None,
            )
            log.info(f"Snapshot: {snap.total_rows} rows | {snap.dashboard_type} | {len(snap.visual_types)} visual types")

            # Reconstruct DataFrame from snapshot for downstream use
            if snap.total_rows > 0:
                # Rebuild df from agg blocks row data if needed
                from backend.ingestion.visual_extractor import VisualDashboardExtractor
                # Re-extract raw df for universal answerer
    ...
    if df.empty and (snap is None or snap.total_rows == 0):
        raise HTTPException(422, "Extracted data is empty. Check view_id and permissions.")

    # ── Auto-analyse schema ──────────────────────────────────────────────
    profile = _analyser.analyse(df if not df.empty else pd.DataFrame(), source_id=source_id, source_name=source_name)

    # ── Generate questions from schema ───────────────────────────────────
    ...
    # Store in-process cache (df + profile + visual snapshot)
    _session_cache[source_id] = {
        "df":      df,
        "profile": profile,
        "snap":    snap,   # DashboardSnapshot from visual extractor
        "meta":    {
            "view_id":        view_id,
            "source_name":    source_name,
            "connected_at":   time.strftime("%Y-%m-%d %H:%M:%S"),
            "dashboard_type": snap.dashboard_type if snap else profile.dashboard_type,
        },
    }

    elapsed = round(time.time() - t0, 2)
    dash_type = snap.dashboard_type if snap else profile.dashboard_type
    log.info(f"connect_dashboard: {source_id} | {dash_type} | {elapsed}s")

    # Build visual types summary for response
    vt_summary = []
    if snap and snap.visual_types:
        vt_summary = [
            {"type": v.vtype, "confidence": round(v.confidence, 2),
             "dims": v.dim_cols[:3], "measures": v.measure_cols[:3]}
    ...
    return {
        ...
        "confidence":         snap.visual_types[0].confidence if snap and snap.visual_types else profile.confidence,
        "total_rows":         snap.total_rows if snap else profile.total_rows,
        "total_cols":         snap.total_cols if snap else profile.total_cols,
        "dimensions":         profile.dimensions,
        "measures":           profile.measures,
        "time_cols":          profile.time_cols,
        "status_cols":        profile.status_cols,
        "kpi_count":          len(profile.kpi_patterns),
        "has_time_series":    snap.has_time      if snap else profile.has_time_series,
        "has_rag_status":     snap.has_rag       if snap else profile.has_rag_status,
        "has_thresholds":     snap.has_threshold if snap else profile.has_thresholds,
        "has_variance":       profile.has_variance,
        "has_multi_measure":  snap.has_multi_measure if snap else False,
        "has_geo":            snap.has_geo if snap else False,
        "visual_types":       vt_summary,
        "agg_blocks":         len(snap.agg_blocks) if snap else 0,
        "questions":          questions,
